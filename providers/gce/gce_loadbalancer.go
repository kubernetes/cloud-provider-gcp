...
func NewLBSyncResult() *LBSyncResult {
	return &LBSyncResult{Annotations: make(map[string]string)}
}
...
func (g *Cloud) updateL4ResourcesAnnotations(ctx context.Context, svc *v1.Service, newL4LBAnnotations map[string]string) error {
	newObjectMeta := computeNewAnnotationsIfNeeded(svc, newL4LBAnnotations, L4ResourceAnnotationKeys)
	if newObjectMeta == nil {
		return nil
	}
	return g.PatchServiceObjectMetadata(ctx, svc, *newObjectMeta)
}

// PatchServiceObjectMetadata patches the given service's metadata based on new
// service metadata.
func (g *Cloud) PatchServiceObjectMetadata(ctx context.Context, svc *v1.Service, newObjectMetadata metav1.ObjectMeta) error {
	newSvc := svc.DeepCopy()
	newSvc.ObjectMeta = newObjectMetadata
	_, err := g.PatchService(ctx, svc, newSvc)
	return err
}
...
func (g *Cloud) PatchService(ctx context.Context, oldSvc, newSvc *v1.Service) (*v1.Service, error) {
	...
	return g.client.CoreV1().Services(oldSvc.Namespace).Patch(ctx, oldSvc.Name, types.StrategicMergePatchType, patchBytes, metav1.PatchOptions{}, "status")
}
