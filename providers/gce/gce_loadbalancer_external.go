...
func (g *Cloud) ensureExternalLoadBalancer(clusterName string, clusterID string, apiService *v1.Service, existingFwdRule *compute.ForwardingRule, nodes []*v1.Node) (*lbSyncResult, error) {
	...
	labels, present, err := GetLoadBalancerAnnotationResourceLabels(apiService)
	if err != nil {
		return nil, fmt.Errorf("failed to parse forwarding rule resource labels for load balancer (%s): %w", lbRefStr, err)
	}
	if present {
