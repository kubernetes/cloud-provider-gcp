...
# ManageFirewallRules indicates that the firewall
# rules should be managed by the provider.
  if [[ -n "${MANAGEFIREWALLRULES:-}" ]]; then
    use_cloud_config="true"
    cat <<EOF >>/etc/gce.conf
manage-firewall-rules = ${MANAGEFIREWALLRULES}
EOF
  fi
