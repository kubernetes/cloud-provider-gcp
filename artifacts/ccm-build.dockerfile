...
# Move the relevant cloud controller manager source into the container
RUN mkdir -p ${HOME}/cmd/cloud-controller-manager
COPY cmd/cloud-controller-manager ${HOME}/cmd/cloud-controller-manager
COPY _output/ccm-vendor.tar.gz ${HOME}
RUN cd ${HOME} && tar xvfz ccm-vendor.tar.gz

CMD go build cmd/cloud-controller-manager/controller-manager.go

MAINTAINER Walter Fender <wfender@google.com>
