...
type mockScaleUpNotifier struct {
	mu           sync.Mutex
	enqueueCount int
}

func (m *mockScaleUpNotifier) Enqueue() {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.enqueueCount++
}

func (m *mockScaleUpNotifier) getEnqueueCount() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.enqueueCount
}

func TestIPAMEngine_AllocatePodIP(t *testing.T) {
	logger := klog.Background()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_test.sqlite")
	...
	resp, err := engine.AllocatePodIP(context.Background(), req)
	if err != nil {
...
func TestIPAMEngine_AllocatePodIP_Concurrency(t *testing.T) {
	logger := klog.Background()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_concurrency_test.sqlite")
...
func TestIPAMEngine_DeallocatePodIP(t *testing.T) {
	logger := logr.Discard()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_test_release.sqlite")
	...
	allocResp, err := engine.AllocatePodIP(context.Background(), reqAlloc)
	if err != nil {
	...
	deallocResp, err := engine.DeallocatePodIP(context.Background(), reqDealloc)
	if err != nil {
...
func TestIPAMEngine_AllocatePodIP_RetryOnDBError(t *testing.T) {
	logger := klog.Background()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_retry_test.sqlite")
	...
	defer cancel()
	_, err = engine.AllocatePodIP(ctx, req)
...
func TestIPAMEngine_AllocatePodIP_NoRetryOnExhaustion(t *testing.T) {
	logger := klog.Background()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_exhaust_test.sqlite")
...
func TestIPAMEngine_AllocatePodIP_IPv6(t *testing.T) {
	logger := klog.Background()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_ipv6_test.sqlite")
...
func TestIPAMEngine_AllocatePodIP_IPv6_Idempotency_Release(t *testing.T) {
	logger := klog.Background()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_ipv6_idempotency_test.sqlite")
	...
	// 1. First allocation
	resp1, err := engine.AllocatePodIP(context.Background(), req)
	if err != nil {
	...
	resp2, err := engine.AllocatePodIP(context.Background(), req)
	if err != nil {
	...
	_, err = engine.DeallocatePodIP(context.Background(), reqDealloc)
	if err != nil {
		t.Fatalf("DeallocatePodIP failed: %v", err)
	}

	// 4. Re-allocate
	resp3, err := engine.AllocatePodIP(context.Background(), req)
	if err != nil {
		t.Fatalf("Re-allocation failed: %v", err)
	}
	// Cooldown is 0 by default, so it should reuse the IP immediately.
...
func TestIPAMEngine_AllocatePodIP_DualStack(t *testing.T) {
	logger := klog.Background()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_dualstack_test.sqlite")
...
func TestIPAMEngine_AllocatePodIP_DynamicAllocation(t *testing.T) {
	logger := klog.Background()

	tests := []struct {
		name         string
		cancelCtx    bool
		action       func(ctx context.Context, engine *IPAMEngine, storeInstance *store.Store, network string) error
		wantErr      bool
		errSubstring string
	...
	}{
		{
			...
			action: func(ctx context.Context, engine *IPAMEngine, storeInstance *store.Store, network string) error {
				...
				engine.OnCIDRAdded(network, 256)
				return nil
			},
			wantErr: false,
			checkResp: func(t *testing.T, resp *adaptiveipam.AllocatePodIPResponse) {
		...
		},
		{
			...
			cancelCtx: true,
			action: func(_ context.Context, _ *IPAMEngine, _ *store.Store, _ string) error {
				return nil
			},
			wantErr:      true,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			tempDir := t.TempDir()
			dbPath := filepath.Join(tempDir, "metis_engine_dynamic_test.sqlite")
			...
			go func() {
				defer wg.Done()
				resp, allocErr = engine.AllocatePodIP(ctx, req)
			}()
			...
			mapLen := engine.GetPendingRequestsCount(network)
			if mapLen != 1 {
				t.Errorf("Expected 1 pending request in requestsMap for network %s, got %d", network, mapLen)
			}
			if notifier.getEnqueueCount() == 0 {
				t.Errorf("Expected scaleUpNotifier.Enqueue() to be called")
			}

			if tc.cancelCtx {
				cancel()
			} else {
				if err := tc.action(ctx, engine, storeInstance, network); err != nil {
			...
			finalMapLen := engine.GetTotalPendingRequestsCount()
			if finalMapLen != 0 {
...
func TestIPAMEngine_AllocatePodIP_DynamicAllocation_MultipleRequests(t *testing.T) {
	logger := klog.Background()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_dynamic_multi_test.sqlite")
	...
	for i := 0; i < numRequests; i++ {
		go func(index int) {
			...
			defer cancel()
			resps[index], errs[index] = engine.AllocatePodIP(ctx, req)
		}(i)
	}
	...
	if engine.GetPendingRequestsCount(network) != 5 {
		t.Errorf("Expected 5 pending requests, got %d", engine.GetPendingRequestsCount(network))
	}
	if notifier.getEnqueueCount() == 0 {
		t.Errorf("Expected scaleUpNotifier.Enqueue() to be called")
	}
	...
	if engine.GetPendingRequestsCount(network) != 0 {
		t.Errorf("Expected 0 pending requests left, got %d", engine.GetPendingRequestsCount(network))
	}
}

func TestIPAMEngine_CheckPodIP(t *testing.T) {
	logger := logr.Discard()
	tempDir := t.TempDir()
	dbPath := filepath.Join(tempDir, "metis_engine_check_test.sqlite")
	...
	_, err = engine.CheckPodIP(context.Background(), req)
	if err == nil {
	...
	_, err = engine.AllocatePodIP(context.Background(), reqAlloc)
	if err != nil {
		t.Fatalf("AllocatePodIP failed: %v", err)
	}

	// 3. Check after allocation (should succeed)
	resp, err := engine.CheckPodIP(context.Background(), req)
	if err != nil {
...
func TestIPAMEngine_AllocatePodIP_Validation(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := engine.AllocatePodIP(context.Background(), req)
			if err == nil {
...
func TestIPAMEngine_DeallocatePodIP_Validation(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := engine.DeallocatePodIP(context.Background(), req)
			if err == nil {
				t.Fatal("Expected error, got nil")
			}
			st, ok := status.FromError(err)
