...
// ToCNIError converts an error into a CNI specification-compliant *types.Error.
func ToCNIError(err error, fallbackMsg string) *types.Error {
	...
	for _, detail := range st.Details() {
		if errorInfo, ok := detail.(*genproto.ErrorInfo); ok && errorInfo.Domain == metiserrors.MetisErrorDomain {
			...
			case metiserrors.ReasonIPPoolExhausted:
				cniCode = types.ErrTryAgainLater // CNI Code 11
				msg = "IP address pool exhausted"
			}
