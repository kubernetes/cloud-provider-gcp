...
// ErrorInterceptor intercepts unary gRPC requests and formats MetisError
// into google.rpc.ErrorInfo status.
func (s *adaptiveIpamServer) ErrorInterceptor(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
	...
	internalErr := metiserrors.ErrInternal(err.Error(), err)
	return nil, internalErr.ToGRPCStatus().Err()
}
