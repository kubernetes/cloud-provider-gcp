...
func (e *NoKindMatchError) Error() string {
	searchedVersions := sets.Set[string]{}
	...
	case 1:
		return fmt.Sprintf("no matches for kind %q in version %q", e.GroupKind.Kind, searchedVersions.UnsortedList()[0])
	default:
		return fmt.Sprintf("no matches for kind %q in versions %q", e.GroupKind.Kind, searchedVersions.UnsortedList())
	}
}
