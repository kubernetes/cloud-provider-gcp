...
// NodeControllerStarter implements the framework.ControllerStarter interface.
type NodeControllerStarter struct {
...
// NewNodeControllerStarter creates a new NodeControllerStarter.
func NewNodeControllerStarter(
	clientBuilder cloudprovider.ControllerClientBuilder,
	kubeClient clientset.Interface,
	mainInformerFactory informers.SharedInformerFactory,
	config *cloudcontrollerconfig.CompletedConfig,
	controlCtx controllermanagerapp.ControllerContext,
	controllers map[string]ControllerStartFunc,
) *NodeControllerStarter {
	return &NodeControllerStarter{
...
func (s *NodeControllerStarter) StartController(pc *v1.ProviderConfig) (chan<- struct{}, error) {
	pcKey := pc.Name
