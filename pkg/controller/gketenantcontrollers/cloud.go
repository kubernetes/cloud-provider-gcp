...
func generateCloudConfig(configFile *gce.ConfigFile, providerConfig *v1.ProviderConfig) (*gce.CloudConfig, error) {
	...
	if providerConfig.Spec.NetworkConfig.Network != "" {
		...
		if strings.Contains(providerConfig.Spec.NetworkConfig.Network, "/") {
			gceCloudConfig.NetworkURL = providerConfig.Spec.NetworkConfig.Network
		}
	}
	...
	if providerConfig.Spec.NetworkConfig.SubnetInfo.Subnetwork != "" {
		...
		parts := strings.Split(subnetworkName, "/")
		if len(parts) > 1 {
			subnetworkName = parts[len(parts)-1]
		}
		gceCloudConfig.SubnetworkName = subnetworkName
		if strings.Contains(providerConfig.Spec.NetworkConfig.SubnetInfo.Subnetwork, "/") {
			gceCloudConfig.SubnetworkURL = providerConfig.Spec.NetworkConfig.SubnetInfo.Subnetwork
		}
	}
...
func updateTokenProjectNumber(tokenBody string, projectNumber int) (string, error) {
	if tokenBody == "" {
		return "", nil // Or error? Original logic allowed empty? If existing is empty, we return empty.
	}

	// Try to unmarshal.
	var bodyMap map[string]any
	// If it fails, maybe it IS quoted?
	// But gcfg usually handles key="value".
	// If the user put key="\"JSON\"", gcfg gives "\"JSON\""?
	// We'll handle both just in case.

	jsonStr := tokenBody
	// Check if it looks like a quoted string that contains JSON
	...
	if err := json.Unmarshal([]byte(jsonStr), &bodyMap); err != nil {
		// If unmarshal fails on unquoted string, and we didn't quote-check it?
		// Or if it WAS quoted but we failed to unquote?
