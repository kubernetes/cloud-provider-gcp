...
graph TD
    subgraph Main["Main Process"]
        CCM["GKE Tenant Controller Manager"] -->|Watches| PC["ProviderConfig CRD"]
        CCM -->|Manages| MM["Provider Config Controller"]
    end

    subgraph PerTenant["Per-Tenant Isolation"]
        MM -->|Spawns| T1["Tenant 1 Controller Loop"]
        MM -->|Spawns| T2["Tenant 2 Controller Loop"]
        
        subgraph Tenant1["Tenant 1 Loop"]
            T1 -->|Creates| C1["Scoped GCE Cloud for Tenant 1"]
            T1 -->|Creates| I1["Filtered Informer Factory"]
            I1 -->|Filters by| L1["Label: provider-config=tenant-1"]
            I1 -->|Feeds| NC1["Node Controller"]
            I1 -->|Feeds| IPAM1["Node IPAM Controller"]
            I1 -->|Feeds| LC1["Node Lifecycle Controller"]
            
            NC1 -->|Talks to| GCE1["GCE API (Project A)"]
            IPAM1 -->|Talks to| GCE1
            LC1 -->|Talks to| GCE1
        end

        subgraph Tenant2["Tenant 2 Loop"]
             T2 -->|Creates| C2["Scoped GCE Cloud for Tenant 2"]
             T2 -->|Creates| I2["Filtered Informer Factory"]
             I2 -->|Filters by| L2["Label: provider-config=tenant-2"]
             I2 -->|Feeds| NC2["Node Controller"]
             I2 -->|Feeds| IPAM2["Node IPAM Controller"]
             I2 -->|Feeds| LC2["Node Lifecycle Controller"]

             NC2 -->|Talks to| GCE2["GCE API (Project B)"]
             IPAM2 -->|Talks to| GCE2
             LC2 -->|Talks to| GCE2
        end
    end
