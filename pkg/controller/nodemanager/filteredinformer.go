...
func (f *FilteredInformer) filterFunc(obj interface{}) bool {
	...
	klog.Infof("FilterFunc: Obj=%q, LabelKey=%q, ExpectedVal=%q, ActualVal=%q (exists=%v), AllowMissing=%v -> Accepted=%v",
		accessor.GetName(), f.providerConfigLabel, f.providerConfigName, val, ok, f.allowMissing, result)
