...
func ControlPlaneUpgradeFunc(f *framework.Framework, upgCtx *UpgradeContext, testCase *junit.TestCase, controlPlaneExtraEnvs []string) func(ctx context.Context) {
...
func ClusterUpgradeFunc(f *framework.Framework, upgCtx *UpgradeContext, testCase *junit.TestCase, controlPlaneExtraEnvs, nodeExtraEnvs []string) func(ctx context.Context) {
	return func(ctx context.Context) {
		target := upgCtx.Versions[1].Version.String()
...
func ClusterDowngradeFunc(f *framework.Framework, upgCtx *UpgradeContext, testCase *junit.TestCase, controlPlaneExtraEnvs, nodeExtraEnvs []string) func(ctx context.Context) {
	return func(ctx context.Context) {
		target := upgCtx.Versions[1].Version.String()
		image := upgCtx.Versions[1].NodeImage
...
func controlPlaneUpgrade(ctx context.Context, f *framework.Framework, v string, extraEnvs []string) error {
	...
	case "gke":
		return MasterUpgradeGKE(ctx, f.Namespace.Name, v)
	default:
...
func nodeUpgradeGCE(rawV, img string, extraEnvs []string) error {
	...
	if img != "" {
		...
		_, _, err := framework.RunCmdEnv(env, GCEUpgradeScript(), "-N", "-o", v)
		return err
	}
	_, _, err := framework.RunCmdEnv(env, GCEUpgradeScript(), "-N", v)
	return err
}
